...
<form action="admin.php?action=createPost" method="post" name="formulaire" id="formulaire">
            ...
            <input type="number" id="id_ep" name="id_ep" min="1" max="1000" required placeholder="Numéro de l'épisode" value="<?= $post['numero'] ?>">
            <label for="texte"></label>
            <textarea id="texte" name="texte" style="with: 60%;" rows="15"><?= $post['content'] ?></textarea>
            <input title="Confirmation d'un nouveau billet" name="send" type="submit" value="Envoyer" id="butForm" class="btn btn-primary"/>
            </form>
